...
define('PATH', '../../');
$message = array();
if(isset($_POST['user_id'],$_POST['pseudo'],$_POST['password'])){
    ...
    $user = new User($user_id,null,$pseudo,null,$password,null,null,null,null,null,null,null,null,null);
    $user_dao = new UserDAO();
    if($user_dao->check_password($user)){
        if($user->isPseudoCorrect())
            $message = ($user_dao->update_pseudo($user)) ? array_merge($message, array('code' => 1)) : array_merge($message, array('code' => -3));   
        else
