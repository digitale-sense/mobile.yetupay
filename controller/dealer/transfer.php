...
if(isset($_GET['user_id'],$_GET['password'],$_GET['receiver_login'],$_GET['amount'],$_GET['currency'])){
    ...
    if($user_dao->check_password($sender)){
        ...
        $dealer = $dealer_dao->get_dealer_by_user_id($sender->getId());
        $amount = $_GET['amount'];
        $currency = $_GET['currency'];
        $sold_available = ($currency == "USD") ? $dealer->getDealerUSDSold() : $dealer->getDealerCDFSold();
        if($sold_available < $amount)
            ...
            if(!is_null($receiver_id) && $receiver_id>0){
                $receiver = $user_dao->get_user_by_id($receiver_id);
                $transfer_dao = new TransferDAO();
                $transfer_dao->transfer($dealer,$receiver,$amount,$currency);
            }
            else
